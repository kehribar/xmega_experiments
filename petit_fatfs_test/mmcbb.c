...
#define DO_INIT()	setInternalPullup(C,6)  /* Initialize port for MMC DO as input */
#define DO			digitalRead(C,6)	    /* Test for MMC DO ('H':true, 'L':false) */

#define DI_INIT()	pinMode(C,5,OUTPUT)	    /* Initialize port for MMC DI as output */
#define DI_H()		digitalWrite(C,5,HIGH)	/* Set MMC DI "high" */
#define DI_L()		digitalWrite(C,5,LOW)	/* Set MMC DI "low" */

#define CK_INIT()	pinMode(C,7,OUTPUT)	    /* Initialize port for MMC SCLK as output */
#define CK_H()		digitalWrite(C,7,HIGH)	/* Set MMC SCLK "high" */
#define	CK_L()		digitalWrite(C,7,LOW)	/* Set MMC SCLK "low" */

#define CS_INIT()	pinMode(C,4,OUTPUT)	    /* Initialize port for MMC CS as output */
#define	CS_H()		digitalWrite(C,4,HIGH)	/* Set MMC CS "high" */
#define CS_L()		digitalWrite(C,4,LOW)	/* Set MMC CS "low" */
...
{
	/* xmega128a4 pinout */
	pinMode(C,7,OUTPUT); // SCK
	pinMode(C,6,INPUT);  // MISO
	pinMode(C,5,OUTPUT); // MOSI
	pinMode(C,4,OUTPUT); // SS

	/* pullup for MISO line */
	setInternalPullup(C,6);
	setInternalPullup(C,5);
	setInternalPullup(C,4);
	
	SPIC.CTRL = SPI_ENABLE_bm|SPI_MASTER_bm|SPI_MODE_0_gc|SPI_CLK2X_bm|SPI_PRESCALER_DIV4_gc;	
}
